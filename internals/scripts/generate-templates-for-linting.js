...
/**
 * Every generated component/feature is preceded by this
 * @type {string}
 */
...
/**
 * Test the feature generator and rollback when successful
 * @param {string} name - Feature name
 * @param {string} type - Plop Action type
 * @returns {Promise<string>} - Relative path to the generated feature
 */
async function generateFeature({ name, memo }) {
  const targetFolder = 'features';
  const componentName = `${NAMESPACE}Feature${name}`;
  const relativePath = `${targetFolder}/${componentName}`;
  const feature = `feature/${memo ? 'Pure' : 'NotPure'}`;
  ...
  await featureGen
    ...
    .then(handleResult)
    .then(feedbackToUser(`Generated '${feature}'`))
    .catch(reason => reportErrors(reason));
  await runLintingOnDirectory(relativePath)
    .then(reportSuccess(`Linting test passed for '${feature}'`))
    .catch(reason => reportErrors(reason));
  await removeDir(relativePath)
    .then(feedbackToUser(`Cleanup '${feature}'`))
    .catch(reason => reportErrors(reason));
...
async function generateComponents(components) {
  const promises = components.map(async component => {
    ...
    } else if (component.kind === 'feature') {
      result = await generateFeature(component);
    }
...
(async function() {
  await generateComponents([
    ...
    { kind: 'feature', name: 'Feature', memo: false },
    { kind: 'feature', name: 'MemoizedFeature', memo: true },
  ]).catch(reason => reportErrors(reason));
